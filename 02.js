...
//
// bartMan says this is the original code, but i suck at opcode port
...
//
// init walkers, inspired by http://chipflip.org/04
//
...
function drawCanvas() {
	...
	function drawThis() {
		...
		switch(pattern) {
			...
			case 16:{
				...
				//var pad = h*(0.02);
				//ctx.clearRect(0,h*0.42,w,h*0.16);
				...
				if (over) {
					...
					if (pattern<2) {
						...
						if (note<32) {
							//ctx.font="bold "+fl((w/800)*30)+"pt Arial";
			...
			case 15: {
				...
				for (var i=0; i<numx; i++) {
					for (var j=0; j<numy; j++) {
						...
						var sqr = size/2*Q(3);
						ctx.save();
						//var sircus = si(timer/2000)*size*.25;
						ctx.ta(i*(w/numx), j*(h/numy));
						ctx.globalCompositeOperation = 'xor';
	...
	/*	ctx.fillStyle = "rgba(0,255,0,1)";
		ctx.fillText(pattern+" "+note, w*0.05, h*0.05);
	*/	
